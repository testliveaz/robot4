...
$(document).ready(() => {
    setTimeout(function () {
        let targetLives = ['wallviral', 'clickaz']; // Bağlanmak istediğiniz kullanıcı adları
        connect(targetLives);
    }, 5000);
});

function connect(usernamess) {
    usernamess.forEach(username => {
        if (username !== '') {
            let connection = new TikTokIOConnection(undefined);

            $('#stateText').append('Bağlanılıyor: ' + username + '...');
            $("#usernameTarget").append("@" + username + "<br/>");

            connection.connect(username, {
                enableExtendedGiftInfo: true
            }).then(state => {
                $('#stateText').append(`Bağlandı: ${state.roomId} (${username})<br/>`);
            }).catch(errorMessage => {
                $('#stateText').append(`Hata: ${errorMessage} (${username})<br/>`);
            });
