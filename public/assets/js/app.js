...
const selectionWeights = new Map(messages.map(msg => [msg.text, 1]));
const WEIGHT_DECAY = 0.5; // Seçildikten sonra ağırlığı azaltma oranı
const RESTORE_INTERVAL = 60000; // Ağırlıkları eski haline getirme süresi (milisaniye)
const MESSAGE_INTERVAL = 20000; // Mesaj ekleme aralığı (milisaniye)

function addRandomMessage() {
    let totalWeight = Array.from(selectionWeights.values()).reduce((a, b) => a + b, 0);
    let randomWeight = Math.random() * totalWeight;
    let selectedMessage;

    for (const [text, weight] of selectionWeights.entries()) {
        if (randomWeight < weight) {
            selectedMessage = messages.find(msg => msg.text === text);
            break;
        }
        randomWeight -= weight;
    }

    selectionWeights.set(selectedMessage.text, selectionWeights.get(selectedMessage.text) * WEIGHT_DECAY);

    messagesQueue.push({ ...selectedMessage, type: 'random' });
    processQueue();
}

setInterval(() => {
    for (const key of selectionWeights.keys()) {
        selectionWeights.set(key, 1);
    }
}, RESTORE_INTERVAL);

setInterval(addRandomMessage, MESSAGE_INTERVAL);
